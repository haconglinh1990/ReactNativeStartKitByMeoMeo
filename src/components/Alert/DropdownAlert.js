...
import { deviceHeight } from '../../config/Layout';
...
export default class DropdownAlert extends Component {
  ...
  onLayoutEvent(event) {
    ...
    } else if (this.props.startDelta > deviceHeight) {
      actualStartDelta = deviceHeight + height;
    }
    if (this.props.endDelta < 0) {
      actualEndDelta = 0;
    } else if (this.props.endDelta > deviceHeight) {
      actualEndDelta = deviceHeight - height;
    }
    var heightDelta = deviceHeight - this.props.endDelta - height;
    if (heightDelta < 0) {
  ...
  renderDropDown(isOpen) {
    if (isOpen == true) {
      ...
      return (
        ...
        >
          <TouchableHighlight
            onPress={this.props.showCancel ? this.props.onPress : () => this.onClose("tap")}
